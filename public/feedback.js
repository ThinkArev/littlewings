...
ratingContainers.forEach((container) => {
  ...
  emojis.forEach((emoji, index) => {
    emoji.addEventListener("click", () => {
      ...
      // Hide error message
      const errorElement =
        container.parentElement.querySelector(".rating-error");
      if (errorElement) {
...
function updateProgress() {
  ...
  if (!anonymousCheckbox.checked) {
    const personalRequiredFields =
      personalInfoDiv.querySelectorAll("input[required]");
  ...
  .addEventListener("submit", function (e) {
    ...
    if (missingRatings.length > 0) {
      ...
      const firstMissing = document.querySelector(
        `.emoji-rating[data-rating="${missingRatings[0]}"]`
      );
      if (firstMissing) {
    ...
    const yourEmail = "littlewingsplayschool25@gmail.com";
